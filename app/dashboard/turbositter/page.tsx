...
export default function TurboSitterPage() {
  ...
  const fetchWeatherData = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/Boltwood');
  ...
  const fetchLatestImage = () => {
    ...
    const url = `/api/PTZ?ts=${Date.now()}`; // avoid caching
    setLatestImage(url);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchCentralEnclosureData, 30000); // refresh every 30s
    return () => clearInterval(interval);
  }, []);
